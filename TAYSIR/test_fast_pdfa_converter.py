...
from fast_pdfa import FastProbabilisticDeterministicFiniteAutomaton
from faster_pdfa import FasterProbabilisticDeterministicFiniteAutomaton
from fast_pdfa_converter import FastProbabilisticDeterministicFiniteAutomatonConverter
...
class TestFastPDFAConverter():
    ...
    def test_speed(self):
        ...
        pdfa = pdfa_generator.pdfa_from_dfa(dfa, terminal_symbol="3")
        fast_pdfa = FastProbabilisticDeterministicFiniteAutomatonConverter().to_fast_pdfa(pdfa)
        faster_pdfa = FastProbabilisticDeterministicFiniteAutomatonConverter().to_faster_pdfa(pdfa)
        symbols = sorted(list(pdfa.alphabet.symbols))
        symbols = symbols + [pdfa.terminal_symbol]
        int_symbols = [int(x.value) for x in symbols]
        sequences = UniformWordSequenceGenerator(
            pdfa.alphabet, 100).generate_words(500)
        ...
        for i in range(10):
            for seq_int in seqs_int:
                faster_pdfa_prob = faster_pdfa.sequence_probability(seq_int)
        end = time.time()
        print(end - start)
        print("A")
