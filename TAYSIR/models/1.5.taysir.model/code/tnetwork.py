...
class TNetwork(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.hides_pairs = type(self.cell_type(1, 1, 1)(torch.zeros(1, 1))[1]) is tuple
        ...
        for i in range(self.n_layers - 1):
            self.mach.append(
                self.cell_type(
                    self.neurons_per_layer * bidimul,
                    self.neurons_per_layer * bidimul,
                    1,
                    dtype=dtype,
                    device=device,
        ...
        # Add a dropout layer
        if self.dropout:
            layers.append(nn.Dropout(self.dropout, dtype=dtype, device=device))
        ...
        layers.append(layer_type(in_features, out_features, dtype=dtype, device=device))
        # Possibly add another fully-connected layer
        if self.split_dense:
            out_features = self.n_outputs
            in_features = self.neurons_per_layer // 2
            layers.append(
                self.final_type(in_features, out_features, dtype=dtype, device=device)
            )
        # Add an activation layer
        if activation is not None:
            layers.append(activation.to(dtype=dtype, device=device))
    ...
    def forward_bin(self, x, hidden=None, full_ret=False):
        ...
        states = [
            self._pass_recurrent(word, hidden, b=b) for b, word in enumerate(words)
        ]
    ...
    def one_hot_encode(self, word):
        tensor = torch.zeros(
            len(word), self.input_size, dtype=self.dtype, device=self.device
        )
    ...
    def predict_lm(self, x):
        ...
        if x.size(-1) != self.input_size:
            if x.size(-1) != self.input_size - 1:
                raise ValueError(
                    f"Need {self.input_size} input features "
                    f"but received {x.size(-1)}"
                )
            z = torch.zeros(*x.size()[:-1], 1, dtype=self.dtype, device=self.device)
            x = torch.cat((z, x), -1)
        t = self.task
    ...
    def _interpret_states(self, states):
        ...
        if self.hides_pairs:
            ...
            hidden = (
                [torch.stack([x[0] for x in state], 1) for state in states],
                [torch.stack([x[1] for x in state], 1) for state in states],
            )
...
def pad(x, max_len):
    ...
    if s > max_len:
        raise ValueError(
            "Sequence too long.  Should be at most"
            f" {max_len} but is actually {x.size(-2)}."
        )
    z = torch.zeros(
        *x.size()[:-2], max_len - s, x.size(-1), dtype=x.dtype, device=x.device
    )
    z[..., 0] = 1
