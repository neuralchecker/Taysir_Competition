...
class LSTMx(torch.nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int,
        ...
        if dropout > 0 and num_layers == 1:
            warnings.warn("dropout option adds dropout after all "
    ...
    def forward_x(self, x, hxcx=None):
        ...
        if hxcx is None:
            ...
            cx = torch.zeros(self.num_layers * num_directions,
                             batch_size, self.hidden_size,
                             dtype=x.dtype, device=x.device)
            hxcx = hx, cx
        if x.dim() == 3:
        ...
        hx, cx = hxcx
        hx = hx.view(num_directions, self.num_layers,
                     batch_size, self.hidden_size)
        cx = cx.view(num_directions, self.num_layers,
                     batch_size, self.hidden_size)
        if x.dim() == 2:
            hx = hx.squeeze(2)
            cx = cx.squeeze(2)
